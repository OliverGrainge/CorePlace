...
class HardnessSampler(CorePlaceStep):
    ...
    def _filter_num_classes(
        self, dataconfig: pd.DataFrame, num_classes: int, hardest_first: bool = True
    ):
    ...
    def _filter_num_instances_per_class(
        ...
        filtered = dataconfig.groupby("class_id", group_keys=False).head(
            num_instances_per_class
        )
        return filtered

    def _filter_num_instances(
        self, dataconfig: pd.DataFrame, num_instances: int, hardest_first: bool = True
    ):
        dataconfig = dataconfig.sort_values(by="hardness", ascending=not hardest_first)
    ...
    def run(self, pipe_state: dict) -> dict:
        ...
        if self.num_classes is not None:
            dataconfig = self._filter_num_classes(
                dataconfig, self.num_classes, self.hardest_first
            )
        if self.num_instances_per_class is not None:
            dataconfig = self._filter_num_instances_per_class(
                dataconfig, self.num_instances_per_class, self.hardest_first
            )
        if self.num_instances is not None:
            ...
            dataconfig = self._filter_num_instances(
                dataconfig, self.num_instances, self.hardest_first
            )
        if self.min_instances_per_class is not None:
            dataconfig = self._filter_min_instances_per_class(
                dataconfig, self.min_instances_per_class, self.hardest_first
            )
